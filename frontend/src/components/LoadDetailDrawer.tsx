...
export function LoadDetailDrawer({
    ...
    const rpm = load.totalRpmNum;
    const delta = Math.max(0, rpm - rcpm);
    const isProfitable = rpm >= rcpm + margin;
    const isBreakEven = Math.abs(rpm - rcpm) <= margin;

    // Calculate contribution
    const contribution = delta * load.distanceNum;

    // Calculate coverage percentage
    const coveragePercent = (contribution / weeklyFixed) * 100;
        ...
        <div className="bg-white border border-border rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="bg-accent border-b border-border p-4">
                <div className="flex items-center gap-3">
                    <button
                        onClick={onClose}
                        className="p-2 rounded-full bg-white text-primary shadow-sm hover:bg-white/80 transition"
                        aria-label="Back to available loads"
                    >
                        <ChevronLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <p className="text-sm text-muted-foreground">Load Details</p>
            ...
            <div className="p-4 space-y-4">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div
                                        ...
                                        style={{
                                            width: `${Math.min((rpm / (rcpm || 1)) * 100, 100)}%`,
                                        }}
                                    />
                                </div>
                            </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs text-blue-700">
                                            To cover your weekly fixed costs, you'd need about{' '}
                                            {Math.ceil(weeklyFixed / delta).toLocaleString()} miles
                                            at this contribution rate
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-2 gap-3 pb-4">
                    ...
                    <Button className="bg-orange-500 hover:bg-orange-600">Book Load</Button>
                </div>
            </div>
        </div>
    );
